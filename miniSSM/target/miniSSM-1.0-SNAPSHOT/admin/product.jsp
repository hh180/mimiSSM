                        ...
                        <c:forEach items="${info.list}" var="p">
                            <tr>
                                ...
                                <td>${p.pNumber}</td>
                                    ...
                                    <%--&nbsp;&nbsp;&nbsp;<a href="${pageContext.request.contextPath}/admin/product?flag=getProductById&pid=${p.pId}">修改</a></td>--%>
                                <td>
                                    <button type="button" class="btn btn-info "
                                            onclick="getProductById(${p.pId},${info.pageNum})">编辑
                                    </button>
...
<script type="text/javascript">
    ...
    function deleteBatch() {
            ...
            if(cks.length==0){
                ...
                if(confirm("您确定删除"+cks.length+"条商品吗？")){
                    ...
                    alert(str);
                    //发送ajax请求到服务器端
                    //window.location="${pageContext.request.contextPath}/product/deleteMore.action?pid="+str;
                    $.ajax({
                        url:"${pageContext.request.contextPath}/product/deleteMore.action",
                        data:{"pids":str},
                        type:"post",
                        dataType: "text",
    ...
    function del(pid,page) {
        ...
        if (confirm("确定删除吗")) {
            ...
            //发出ajax的请求，进行删除操作
            $.ajax({
                url:"${pageContext.request.contextPath}/product/delOne.action",
    ...
    //根据id和当前页码查询，回显需要更新的数据
    function getProductById(pid,page) {
        //获取所有多条件查询的条件
        var pname = $("#pname").val();
        var typeid = $("#typeid").val();
        var lprice = $("#lprice").val();
        ...
        //向服务器提交请求，携带这些参数
        location.href = "${pageContext.request.contextPath}/product/getProductById.action"+str;
    }
</script>
<!--分页的AJAX实现-->
<script type="text/javascript">
